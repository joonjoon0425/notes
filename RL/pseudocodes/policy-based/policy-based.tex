...
\section{Metrics for Defining Optimal Policies}\label{sec:Metrics}
    ...
    \begin{enumerate}
        \item average state value: $\bar{v}_\pi = \sum_{s\in\mathcal{S}}d(s)v_\pi(s) = \E_{S \sim d}[v_\pi(S)]$ where $d(s)$ is the weight of state $s$. $d(s)$ can be independent of the policy $\pi$, denoted as $d_0(s)$ and $v_0$, or dependent, for example, stationary distribution.;
...
\section{Policy Gradient Theorem}
    ...
    \begin{lem}{Gradient of $v_\pi(s)$}{grad-valuefunction}
        In the discounted case, the following holds for any $s \in \mathcal{S}$
        \begin{equation}
            \nabla_\theta v_{\pi_\theta}(s) = \sum_{s' \in \mathcal{S}}P_{\pi_\theta}(s'|s) \sum_{a \in \mathcal{A}(s')}\nabla_\theta \pi_\theta(a|s')q_{\pi_\theta}(s',a)
        \end{equation}
        where
        \begin{equation}
            P_{\pi_\theta}(s'|s) = \sum_{k=0}^\infty \gamma^k [P_{\pi_\theta}^k]_{ss'} = [(I_n - \gamma P_{\pi_\theta})^{-1}]_{ss'}
        \end{equation}
        is the discounted total probability of transitioning from $s$ to $s'$ under policy $\pi_\theta$. $[P_{\pi_\theta}^k]_{ss'}$ denotes the probability of transitioning from
        $s$ to $s'$ using exactly $k$ steps under policy $\pi_\theta$.
    \end{lem}
    \begin{proof}
        Try to naively differentiate $v_{\pi_\theta}(s) = \sum_{a \in \mathcal{A}(s)}\pi_\theta(a|s)q_\pi(s,a)$. Then, using the relationship between state value function and action value function,
        you'll be able to derive another $v_{\pi_\theta}$ on the right-hand side. Like the derivation of vector form of Bellman equation, use matrix-vector form to solve the linear equation. For more information, see \cite{Zhao2025}.
    \end{proof}
