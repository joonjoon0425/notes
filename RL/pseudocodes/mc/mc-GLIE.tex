\section{Monte Carlo without Exploring Starts}
        ...
        }
        {
            ...
            \Repeat{convergence}
            {
                ...
                \For{$t=T-1,T-2,\dots,0$}
                {
                    ...
                    $\text{Ret}(s_t,a_t) \leftarrow \text{Ret}(s_t,a_t) + G$\;
                    $\text{N}(s_t,a_t) \leftarrow \text{N}(s_t,a_t) + 1$\;
                    \tcp{policy evaluation}
                    $q(s_t,a_t) \leftarrow \frac{\text{Ret}(s_t,a_t)}{\text{N}(s_t,a_t)}$\;
                    \tcp{policy improvement}
                    $\pi_{k+1}(a_t|s_t) = \begin{cases}
    ...
    \subsection{Implementation Details}
        ...
        According to the approximation theories in Chapter \ref{ch:stochasticApprox}, we can also estimate the state values using the stochastic approximation, without using the Ret$(s,a)$ and N$(s,a)$ in Algorithm \ref{alg:mc-EpsGreedy}.
        The mainly used approximation is to update the action value as
        \begin{equation}
