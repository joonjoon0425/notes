\section{Advantage Actor-Critic (A2C)}
    ...
    \subsection{Implementation Details}
        ...
        To stop the policy from being almost deterministic too quickly, we can add \textbf{entrophy bonus} at the loss function (\cite{pmlr-v48-mniha16}, \cite{WILLIAMS1991}).
        TODO: Study policy gradient theorems for better understanding on policy gradients.
        where 
        \begin{equation}
            \mathcal{H}(\pi(\cdot|s)) = -\int \pi(a|s)\log{\pi(a|s)}da
        \end{equation}
        The discrete entrophy is then $\mathcal{H} = -\sum_a \pi(a|s)\log{\pi(a|s)}$.
