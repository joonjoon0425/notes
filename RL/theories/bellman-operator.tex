...
\section{Generalized Bellman Equation}
    \begin{defn}{Generalized Bellman Equation}{GeneralizedBellmanEq}
        ...
        \begin{equation}
            v(s) = \bigotimes_a \Big(r(s, a) + \gamma \bigoplus_{s'}{}^a v(s') \Big) 
        \end{equation}
    ...
    And we will only use the operator $\Big[\bigoplus_{s'} v\Big]{}^a(s) = \sum_{s' \in \mathcal{S}} p(s'|s,a) v(s')$, since we only cope with
    MDPs.
\section{Bellman Equation}
    ...
    And one can see the operator $\Big[\bigotimes_{s'} v\Big]{}^a(s) = \sum_{s' \in \mathcal{S}} p(s'|s,a) v(s')$.
...
\section{Bellman Optimality Equation}
    ...
    \subsection{Bellman Optimality Equation}
        ...
        \begin{defn}{Bellman Optimality Equation}{BellOptEq}
            Let $\gamma \in (0,1)$ be a discount rate. Then, the Bellman Optimality Equation is defined as
            \begin{align}
...
\section{Solution of The Bellman Equations}
    \subsection{Contraction Mapping Theorem}
        ...
        \begin{proof}
            ...
            The Cauchy sequence has a nice property; the Cauchy sequence always converges. This can be proved by using the properties of $\mathbb{R}$, that the $\mathbb{R}$ is a complete set.
...
\section{Optimal Policy, Optimal State Value and Bellman Optimality Equation}\label{sec:OptPolOptStateValBOE}
    ...
    \begin{thm}{Optimality of $v^*$ and $\pi^*$}{OptPolOptStateVal}
            Let $v^*$ be the solution of BOE $v=T^* v$, and let $\pi^*(a|s) = 1 \Leftrightarrow a \in \arg\max_{a'}q(s,a')$.
            Then, $v^*$ is an optimal state vaule function, and $\pi^*$ is an optimal policy. That is, for any policy $\pi$,
            following holds;
            \begin{equation}
                v^* = v_{\pi^*} \geq v_\pi
            \end{equation}
        ...
        \end{thm}
        \begin{proof}
            \textit{Part 1.} We prove two lemmas.

            First, $v_1 \leq v_2 \Rightarrow T v_1 \leq T v_2$ for $T=T^*_V \text{ or } T^\pi_V$ elementwise.
            $q_1(s,a) = r(s,a) + \gamma \sum_{s'} p(s'|s,a)v_1(s') \leq r(s,a) + \gamma \sum_{s'}p(s'|s,a)v_2(s') = q_2(s, a)$ for all $(s,a)$ since $p(s'|s,a) \geq 0$ and $\gamma > 0$.
            Then, $q_1(s,a) \leq q_2(s,a) \leq \max_a q_2(s, a)$ and $T^* v_1 = \max_a q_1(s,a) \leq T^* v_2 = \max_a q_2(s,a)$, $T^\pi v_1 \leq T^\pi v_2$.

            Second, $\max_{\pi \in \Pi} \sum_{a \in \mathcal{A}(s)} \pi(a|s)q(s,a) = \max_{a \in \mathcal{A}(s)}q(s,a)$. Where 
            \begin{align}
                \sum_{a \in \mathcal{A}(s)} \pi(a|s)q(s,a) &\leq \sum_{a \in \mathcal{A}(s)} \pi(a|s) \max_{a'}{q(s,a')} \\
                                                           &= \max_{a'}{q(s,a')}
            \end{align}
            and since $\mathcal{A}(s)$ is a finite set, there exists $a' \in \arg\max_a q(s,a)$. If we let $\pi(a'|s) = 1$ and 0 otherwise,
            we get $\sum_{a \in \mathcal{A}(s)} \pi(a|s)q(s,a) = q(s,a') = \max_a q(s,a)$. Therefore, $\max_{\pi \in \Pi} \sum_{a \in \mathcal{A}(s)} \pi(a|s)q(s,a) = \max_{a \in \mathcal{A}(s)}q(s,a)$
            and the deterministic policy $\pi$ satisfying the equation exists, which is the policy $\pi^*$. Then, $T^{\pi^*} v^* = T^* v^* = v^*$.
            Since there exists a unique solution $v_{\pi^*}$ to the equation $T^{\pi^*} v =  v$, $v^* = v_{\pi^*}$.
            
            \textit{Part 2.} We show that $v^* \geq v_\pi$ for any policy $\pi$, elementwise. Using the first lemma of Part 1,
            \begin{equation}
                v^* = T^* v^* \geq T^\pi v^* \geq T^\pi T^\pi v^* \geq \dots \geq [T^\pi]^k v^*
            \end{equation}
            for all $k \in \mathbb{N}$, elementwise. Using the Theorem~\ref{thm:ContractMap}, we can see that $[T^\pi]^k v^* \to v_\pi$, as $k \to \infty$,
            where $v_\pi$ is the solution of the Bellman equation $T^\pi v = v$. Therefore, $v^* \geq v_\pi$, and this means that $v^*$ is the optimal value function.
        \end{proof}
    
    The important truth is that the optimal state value is unique but the optimal policy is not. The Theorem~\ref{thm:OptPolOptStateVal}
...
\section{Factors that Influence Optimal Policies}
    ...
    \subsection{Invariance of the Optimal Policy}
        ...
        \begin{thm}{Optimal Policy Invariance}{OptPolInvariance}
            ...
            \begin{equation}
                v' = \alpha v^* + \frac{\beta}{1 - \gamma}\textbf{1}
            \end{equation}
            where $\gamma \in (0,1)$ is the discount rate and $\textbf{1} = [1,\dots,1]^\top$.
